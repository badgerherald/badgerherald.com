...
if (is_admin()) :
	...
	function exa_extra_headlines($post)
	{
		...
		$subhead = exa_get_subhead($post->ID);
...
<div class="aftertitlediv">
    ...
    <hr />
    <label for="_webpress_subhead">Subheading: </label>
    <input id="subhead" class="exa-subhead" type="text" autocomplete="off" value="<?php echo     esc_attr($subhead); ?>"
        name="_webpress_subhead" placeholder="Optional">
</div>
...
<?php
	...
	function exa_subhead_save_postdata($post_id)
	{
		...
		if (!array_key_exists("_webpress_subhead", $_POST)) {
			return;
		}
		...
		$new_subhead = sanitize_text_field($_POST['_webpress_subhead']);

		if ($new_subhead == '') {
			delete_post_meta($post_id, '_webpress_subhead');
		} else {
			update_post_meta($post_id, '_webpress_subhead', $new_subhead);
		}
