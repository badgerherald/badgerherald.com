    ...
    jQuery('#user-meta-hrld_staff_banner_button').click(function() {
        ...
        custom_uploader.on('select', function() {
            ...
            if (preview_img.length == 0)
                jQuery('#user-meta-hrld_staff_banner').parent().append(
                    '<img id="user-meta-hrld_staff_banner_img" src=' + media_attachment.sizes
                    .thumbnail.url +
                    ' /><br /><button type="button" class="button delete_button">Delete Banner</button>'
                    );



            else
                jQuery('#user-meta-hrld_staff_banner_img').attr('src', media_attachment.sizes
                    .thumbnail.url);
...
<table class="form-table">
    ...
    <tr>
        ...
        <td><?php if($field["name"]=="hrld_twitter_handle") echo "@"; ?><input type="<?php echo $field["type"] ?>"
                name="<?php echo $field["name"] ?>" id="user-meta-<?php echo $field["name"] ?>"
                value="<?php echo esc_attr( get_the_author_meta( $field["key"], $user->ID ) ); ?>"
                class="regular-text" /><br /><span class="description"><?php echo $field["description"]; ?></span></td>
    </tr>
    <?php elseif( $field["type"] == 'image') : ?>
    <tr>
        <th><label for="<?php echo $field["name"] ?>"><?php echo $field["title"] ?></label></th>
        <td>
            <input type="hidden" name="<?php echo $field["name"] ?>" id="user-meta-<?php echo $field["name"] ?>"
                value="<?php echo esc_attr( get_the_author_meta( $field["key"], $user->ID ) ); ?>"
                class="regular-text" />
            <button type="button" name="<?php echo $field["name"] ?>-button"
                id="user-meta-<?php echo $field["name"] ?>_button" class="button"
                data-title="Choose Banner Image: <?php echo $user->display_name; ?>" />Choose Image</button><br /><span
                class="description"><?php echo $field["description"]; ?></span><br /><br />
    ...
    <tr>
        ...
        <td>
            ...
            <select multiple name="<?php echo $field["name"].'[]' ?>" id="user-meta-<?php echo $field["name"] ?>"
